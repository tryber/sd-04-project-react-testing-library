...
describe('Requisito 3', () => {
  ...
  it('Não possui Pokemons Favoritos', () => {
    const { getByText } = render(<FavoritePokemons />);
    expect(getByText('No favorite pokemon found')).toBeInTheDocument();
  });

  it('Não deve exibir pokemons não favoritados', () => {
    const { queryByTestId } = render(<FavoritePokemons />);
    expect(queryByTestId('pokemon-name')).not.toBeInTheDocument();
  });

  it('Exibi os pokemons favoritados', () => {
    const { getByTestId } = render(<FavoritePokemons pokemons={favorite} />);
